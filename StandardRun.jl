...
include("./IBM_Functions_WithPatchCorrT")
...
NON=IBM_Functions_WithPatchCorrT.CellCount(xnon,ynon,Ngrid)
TOX=IBM_Functions_WithPatchCorrT.CellCount(xtox,ytox,Ngrid)
Patchiness[1,1]=IBM_Functions_WithPatchCorrT.LloydPatchiness(NON) # Initial patchiness nontoxics
Patchiness[1,2]=IBM_Functions_WithPatchCorrT.LloydPatchiness(TOX) # Initial patchiness nontoxics

Correlation=zeros(Float64,t_end2+1,6)
Correlation[1,:]=IBM_Functions_WithPatchCorrT.CorrAllCombinations(NON,TOX,T,C)
...
PosTox=IBM_Functions_WithPatchCorrT.LinIndex(xtox,ytox,Ngrid)
PosNon=IBM_Functions_WithPatchCorrT.LinIndex(xnon,ynon,Ngrid)
...
@time C, T, xnon, ynon, xtox, ytox, PopDens, Patchiness, Correlation, T_exposed, G_reduced = IBM_Functions_WithPatchCorrT.Sim_Tox_Graz_PDE(C,T,xnon,ynon,xtox,ytox,PopDens,Patchiness,Correlation,T_exposed,G_reduced,Ngrid,t_start1,t_end1,λ,Q,
                                                            rmax_tox,rmax_non,mtox,mnon,Gmax_tox,Gmax_non,HC,HT,qN,dt,kernel_2D,Δ)

## invasion event
if (tmax2!=0)
    ReplacePos=sample(1:length(ynon),100,replace=false) # invasion event: replace 100 non-toxic by toxic cells
    ...
    NON=IBM_Functions_WithPatchCorrT.CellCount(xnon,ynon,Ngrid)
    TOX=IBM_Functions_WithPatchCorrT.CellCount(xtox,ytox,Ngrid)
    Patchiness[t_start2,1]=IBM_Functions_WithPatchCorrT.LloydPatchiness(NON) # Invasion event - overwrite patchiness nontoxics at t_start2
    Patchiness[t_start2,2]=IBM_Functions_WithPatchCorrT.LloydPatchiness(TOX) # Invasion event - overwrite patchiness toxics at t_start2

    Correlation[t_start2,:]=IBM_Functions_WithPatchCorrT.CorrAllCombinations(NON,TOX,T,C) # Invasion event - overwrite Pearson's correlation coefficients at t_start2
    ...
    @time C, T, xnon, ynon, xtox, ytox, PopDens, Patchiness, Correlation, T_exposed, G_reduced = IBM_Functions_WithPatchCorrT.Sim_Tox_Graz_PDE(C,T,xnon,ynon,xtox,ytox,PopDens,Patchiness,Correlation,T_exposed,G_reduced,Ngrid,t_start2,t_end2,λ,Q,
                                                                 rmax_tox,rmax_non,mtox,mnon,Gmax_tox,Gmax_non,HC,HT,qN,dt,kernel_2D,Δ) #,U)
end

# Total Abundance
TotalPopDens=sum(PopDens[1:(t_end2+1),:],dims=2)
...
p1=scatter(xnon.*dx,ynon.*dx,xlabel=L"x\, [cm]",ylabel=L"y\, [cm]",
    ...
    guidefontsize=8,titlefontsize=8,left_margin=2mm)#,right_margin=20mm)
scatter!(xtox.*dx,ytox.*dx,mc=:red,msc=:black,ms=0.8,msw=0.0) #ms=1.2
...
p3=heatmap(dx:dx:L,dx:dx:L,transpose(T)./(dx^2),clims=(0,maximum(T)/(dx^2)),title=L"T-conc.\, [amol\,T\,cm^{-2}]",titlefontsize=8,xlims=(0,L),ylims=(0,L),
    xlab=L"x\,[cm]",ylab=L"y\,[cm]",guidefontsize=8,left_margin=2mm,c=:inferno)
p4=plot(0:dt:(tmax1+tmax2),PopDens[1:(t_end2+1),:],reuse=false,legend=false,xlabel=L"t\, [d]",ylabel=L"Cell\,\, number",
    ...
    plot!(0:dt:(tmax1+tmax2),TotalPopDens,reuse=false,legend=false,linewidth=0.3,linecolor=[:grey])
...
annotate!( -60, 50, text(L"total", :left, 8, color=:black), subplot=2)
annotate!(-65, 49, text(L"\textbf{-}", :left, 20, color=:grey), subplot=2)

# Add invasion plot
t_before=tmax1-50
t_after=tmax1+200
tbef=floor(Int64,t_before/dt)
taft=floor(Int64,t_after/dt)
plot!(t_before:dt:t_after,PopDens[(tbef+1):(taft+1),:],
    inset_subplots = (1, bbox(0.05, 0.15, 0.5, 0.3, :bottom, :right)),subplot=5,
    reuse=false,legend=false,xlabel=L"",ylabel="", xlims=(t_before,t_after),ylim=(0.8*10^2,2*10^4),yaxis=:log,
    xticks = (t_before:50:t_after,["","$tmax1","","","","$t_after"]), yticks = [1e2,1e3,1e4], xtickfontsize=6,ytickfontsize=6,
    linecolor=[:steelblue :red],linewidth=1.0,guidefontsize=4, label = ["non-toxic" "toxic"])
